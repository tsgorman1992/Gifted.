                  ...
                  <AnimatePresence mode="wait">
                    {phase !== "revealed" ? (
                      ...
                      >
                        ...
                        <div
                          className="absolute inset-0"
                          style={{ background: "linear-gradient(160deg, #E8A87C, #D4813A, #b8632a)" }}
                        />

                        {/* Envelope bottom-corner folds */}
                        <div className="absolute inset-0 pointer-events-none overflow-hidden">
                          <div className="absolute inset-0" style={{
                            background: "linear-gradient(to bottom-right, transparent 49.5%, rgba(0,0,0,0.12) 49.5%, rgba(0,0,0,0.12) 50.5%, transparent 50.5%)"
                          }} />
                          <div className="absolute inset-0" style={{
                            background: "linear-gradient(to bottom-left, transparent 49.5%, rgba(0,0,0,0.12) 49.5%, rgba(0,0,0,0.12) 50.5%, transparent 50.5%)"
                          }} />
                        </div>

                        {/* Envelope flap (top triangle) — lifts open when opening */}
                        <motion.div
                          className="absolute top-0 left-0 right-0 pointer-events-none"
                          style={{
                            height: "55%",
                            clipPath: "polygon(0 0, 100% 0, 50% 85%)",
                            background: "linear-gradient(180deg, #c06b2a, #D4813A)",
                            transformOrigin: "top center",
                          }}
                          animate={phase === "opening"
                            ? { rotateX: -170, opacity: 0 }
                            : { rotateX: 0,   opacity: 1 }}
                          transition={{ duration: 0.75, ease: [0.25, 0.1, 0.25, 1] }}
                        />

                        {/* Wax seal + name — fade out when opening */}
                        <motion.div
                          className="absolute inset-0 flex flex-col items-center justify-center gap-2"
                          animate={phase === "opening" ? { opacity: 0, scale: 0.85, y: 10 } : { opacity: 1, scale: 1, y: 0 }}
                          transition={{ duration: 0.4, delay: phase === "opening" ? 0 : 0 }}
                        >
                          <motion.div
                        ...
                        {/* Golden shimmer sweep on opening */}
                        {phase === "opening" && (
                          <motion.div
                            className="absolute inset-0 pointer-events-none"
                            style={{
                              background: "linear-gradient(105deg, transparent 30%, rgba(255,220,130,0.55) 50%, transparent 70%)",
                ...
                <div className="flex-1 p-5 flex flex-col gap-2.5 overflow-hidden">
                  <AnimatePresence mode="wait">
                    ...
                    {phase === "revealed" && (
                      ...
                      >
                        ...
                        {/* Photo strip — real Unsplash images */}
                        <motion.div
                  ...
                  <div className="mt-auto p-3.5 rounded-xl border border-border flex items-center justify-between">
                    <div className="flex items-center gap-2.5">
                      <motion.div
                        animate={phase === "revealed" ? { scale: [1, 1.3, 1] } : {}}
                    ...
                    {phase === "revealed" ? (
                      ...
                      <Button
                        size="sm"
                        variant="secondary"
                        className="rounded-full h-8 text-xs transition-all duration-300"
